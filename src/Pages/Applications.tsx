...
const applyFilters = () => {
  let filtered = [...applications] as JobApplication[];
    ...
    // Remote only filter
    if (filters.remote_only !== "all") {
      if (filters.remote_only === "true") {
        filtered = filtered.filter(app => 
          app.location?.toLowerCase().includes('remote') ||
          app.employment_type?.toLowerCase().includes('remote') ||
          app.job_description?.toLowerCase().includes('remote')
        );
      } else if (filters.remote_only === "false") {
        filtered = filtered.filter(app => 
          !app.location?.toLowerCase().includes('remote') &&
          !app.employment_type?.toLowerCase().includes('remote')
        );
    ...
    if (filters.salary_min || filters.salary_max) {
      filtered = filtered.filter(app => {
        if (!app.salary_range) return true; // Keep jobs without salary info
        
        const salaryMatch = app.salary_range.match(/(\d+)/g);
        ...
        const filterMin = filters.salary_min ? parseInt(filters.salary_min) * 1000 : 0;
        const filterMax = filters.salary_max ? parseInt(filters.salary_max) * 1000 : Infinity;
    ...
    if (filters.location_filter) {
      filtered = filtered.filter(app =>
        app.location.toLowerCase().includes(filters.location_filter.toLowerCase())
      );
    }

    // Bookmarked filter
    if (filters.bookmarked !== "all") {
      const isBookmarked = filters.bookmarked === "true";
      filtered = filtered.filter(app => !!app.is_bookmarked === isBookmarked);
    }
